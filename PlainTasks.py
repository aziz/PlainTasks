...
class PlainTasksBase(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        self.taskpaper_compatible = self.view.settings().get(
            'taskpaper_compatible')
        if self.taskpaper_compatible:
            self.open_tasks_bullet = self.done_tasks_bullet = self.canc_tasks_bullet = '-'
            self.before_date_space = ''
        else:
            self.open_tasks_bullet = self.view.settings().get(
                'open_tasks_bullet')
            self.done_tasks_bullet = self.view.settings().get(
                'done_tasks_bullet')
            self.canc_tasks_bullet = self.view.settings().get(
                'cancelled_tasks_bullet')
            self.before_date_space = ' '
        self.before_tasks_bullet_spaces = ' ' * \
            self.view.settings().get('before_tasks_bullet_margin')
...
class PlainTasksNewCommand(PlainTasksBase):
    ...
    def runCommand(self, edit):
        ...
        # because with multiple selections regions would be messed up after
        # first iteration
        selections.reverse()
        for region in selections:
            ...
            if 'item' in current_scope:
                ...
                if not_empty_line:
                    grps = not_empty_line.groups()
                    line_contents = (grps[0] if len(grps[0]) > 0 else self.before_tasks_bullet_spaces) + \
                    ...
                    line_contents = self.before_tasks_bullet_spaces + \
...
class PlainTasksCompleteCommand(PlainTasksBase):
    ...
    def runCommand(self, edit):
        ...
        try:
            done_line_end = ' %s%s%s' % (self.done_tag, self.before_date_space,
                                         datetime.now().strftime(self.date_format).decode(self.sys_enc))
        except:
            done_line_end = ' %s%s%s' % (
                self.done_tag, self.before_date_space, datetime.now().strftime(self.date_format))
        ...
        for region in self.view.sel():
            ...
            if 'pending' in current_scope:
                ...
                replacement = u'%s%s%s' % (
                    grps[0], self.done_tasks_bullet, grps[1].rstrip())
                self.view.replace(edit, line, replacement)
                if started_matches:
                    self.calc_end_start_time(
                        self, edit, line, started_matches.group(1), done_line_end, eol)
                ...
                self.view.insert(
                    edit, line.begin() + len(indent.group(1)), '%s ' %
                    self.done_tasks_bullet)
            elif 'completed' in current_scope:
                grps = done_matches.groups()
                replacement = u'%s%s%s' % (
                    grps[0], self.open_tasks_bullet, grps[1].rstrip())
                self.view.replace(edit, line, replacement)
                offset = -offset
    ...
    def calc_end_start_time(self, edit, line, started_matches, done_line_end, eol, tag='lasted'):
        ...
        end = datetime.strptime(done_line_end.replace(
            ' @done ', '').replace(' @cancelled ', ''), self.date_format)
        self.view.insert(edit, line.end() + eol, ' @%s(%s)' %
                         (tag, str(end - start)))
...
class PlainTasksCancelCommand(PlainTasksBase):
    ...
    def runCommand(self, edit):
        original = [r for r in self.view.sel()]
        try:
            canc_line_end = ' %s%s%s' % (self.canc_tag, self.before_date_space,
                                         datetime.now().strftime(self.date_format).decode(self.sys_enc))
        except:
            canc_line_end = ' %s%s%s' % (
                self.canc_tag, self.before_date_space, datetime.now().strftime(self.date_format))
        ...
        for region in self.view.sel():
            ...
            if 'pending' in current_scope:
                ...
                replacement = u'%s%s%s' % (
                    grps[0], self.canc_tasks_bullet, grps[1].rstrip())
                self.view.replace(edit, line, replacement)
                if started_matches:
                    PlainTasksCompleteCommand.calc_end_start_time(
                        self, edit, line, started_matches.group(1), canc_line_end, eol, tag='wasted')
            elif 'header' in current_scope:
                eol = self.view.insert(edit, line.end(), canc_line_end)
                if started_matches:
                    PlainTasksCompleteCommand.calc_end_start_time(
...
class PlainTasksArchiveCommand(PlainTasksBase):
    ...
    def runCommand(self, edit):
        ...
        done_tasks = [i for i in self.view.find_by_selector(rds) if i.a < (
            archive_pos.a if archive_pos and archive_pos.a > 0 else self.view.size())]
        for i in done_tasks:
            self.get_task_note(i, done_tasks)

        canc_tasks = [i for i in self.view.find_by_selector(rcs) if i.a < (
            archive_pos.a if archive_pos and archive_pos.a > 0 else self.view.size())]
        for i in canc_tasks:
        ...
        if all_tasks:
            ...
            projects = self.view.find_all(
                '^\s*(\w+.+:\s*(\@[^\s]+(\(.*?\))?\s*)*$\n?)|^\s*---.{3,5}---+$', 0)

            # adding tasks to archive section
            for task in all_tasks:
                match_task = re.match(
                    '^\s*(\[[x-]\]|.)(\s+.*$)', self.view.substr(task), re.U)
                ...
                if rds in current_scope or rcs in current_scope:
                    ...
                    eol = self.before_tasks_bullet_spaces * 2 + \
                        self.view.substr(task).lstrip() + '\n'
                line += self.view.insert(edit, line, eol)

            # remove moved tasks (starting from the last one otherwise it screw
            # up regions after the first delete)
    ...
    def get_task_project(self, task, projects):
        ...
        if index >= 0:
            depth = re.match(
                r"\s*", self.view.substr(self.view.line(task))).group()
            while index >= 0:
                ...
                if prog.match(strProject):
                    ...
                    if len(spaces) < len(depth):
                        hierarhProject = prog.match(strProject).group(
                            2) + ((" / " + hierarhProject) if hierarhProject else '')
                        depth = spaces
                        if len(depth) == 0:
                            break
                else:
...
class PlainTasksOpenLinkCommand(sublime_plugin.TextCommand):
    LINK_PATTERN = re.compile(
        r'\.[\\/](?P<fn>[^\\/:*?"<>|]+)+[\\/]?(>(?P<sym>\w+))?(\:(?P<line>\d+))?(\:(?P<col>\d+))?(\"(?P<text>[^\n]*)\")?', re.I | re.U)
    ...
    def _on_panel_selection(self, selection):
        if selection >= 0:
            ...
            self.opened_file = win.open_file(
                '%s:%s:%s' % res, sublime.ENCODED_POSITION)
        ...
        if sym:
            ...
            for folder in win.folders():
                for root, dirnames, filenames in os.walk(folder):
                    filenames = [os.path.join(root, f) for f in filenames] + [v.file_name()
                                                                              for v in win.views() if v.file_name()]
                    for name in filenames:
                        if name.lower().endswith(fn.lower()):
                            self._current_res.append(
                                (name, line if line else 0, col if col else 0))
    ...
    def run(self, edit):
        ...
        if match:
            fn, sym, line, col, text = match.group(
                'fn', 'sym', 'line', 'col', 'text')
            self.show_panel_or_open(fn, sym, line, col, text)
            if text:
                sublime.set_timeout(
                    lambda: self.find_text(self.opened_file, text, line), 50)

    def find_text(self, view, text, line):
        result = view.find(text, view.sel()
                           [0].a if line else 0, sublime.LITERAL)
        view.sel().clear()
...
class PlainTasksSortByDate(PlainTasksBase):
    ...
    def runCommand(self, edit):
        ...
        if archive_pos:
            ...
            tasks_prefixed_date = []
            tasks = self.view.find_all(
                have_date, archive_pos.b - 1, "\\2\\1", tasks_prefixed_date)
            notes = []
            ...
            tasks_prefixed_date.sort(
                reverse=self.view.settings().get('new_on_top'))
            eol = archive_pos.end()
...
class PlainTaskInsertDate(PlainTasksBase):
    ...
    def runCommand(self, edit):
        for s in reversed(list(self.view.sel())):
            self.view.insert(edit, s.b, datetime.now()
